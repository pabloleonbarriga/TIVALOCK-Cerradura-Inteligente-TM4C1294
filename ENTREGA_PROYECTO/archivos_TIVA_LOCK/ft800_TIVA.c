...
// CONFIGURACION ROTADO PANTALLA 180 GRADOS
HAL_SPI_CSLow();                                // CS low                   Write REG_PCLK to 5
FT800_SPI_SendAddressWR(REG_ROTATE);              // Send the address
...
void cmd_bmp_layout(int type, int lnstrd,int hght){
    ...
    // Calcular el total como la suma de todos los valores
        total = BMP_LAYOUT_HANDLE + BMP_LAYOUT_FMRT_RGB565 + linestride + height;
    else
        total = BMP_LAYOUT_HANDLE + BMP_LAYOUT_FMRT_RGB332 + linestride + height;
...
// Función para mandar por pantalla el comando correspondiente a ALPHA_A. Modifica la transparencia del bitmap
...
void dibujar_imagen_externa(int ram_addr, int ancho, long int alto, int x, int y ,int param, int tipo_RGB){
    ...
    // Comando para BITMAP_LAYOUT   (ft800 prog guide - pg. 88)     || https://www.ftdichip.com/Support/Documents/AppNotes/AN_303%20FT800%20Image%20File%20Conversion.pdf , pg.11
    cmd_bmp_layout(tipo_RGB, ancho, alto);

    // Comando para BITMAP_SIZE     (ft800 prog guide - pg. 93)     || https://www.ftdichip.com/Support/Documents/AppNotes/AN_303%20FT800%20Image%20File%20Conversion.pdf , pg.11
    cmd_bmp_size(ancho, alto);

    // Comando para ALPHA_A         (ft800 prog guide - pg. 117)    || https://www.ftdichip.com/Support/Documents/AppNotes/AN_303%20FT800%20Image%20File%20Conversion.pdf , pg.11
    cmd_alpha_a(param);
...
// Almacena en la memoria ram del FT800 la imagen almacenada en la flash del TM4C mediante LM Flash Programmer
void almacenar_imagen_externa(int flash_addr, int ram_addr, int nbytes_imagen){
    ...
    for (img_ext_indice = 0; img_ext_indice < nbytes_imagen; img_ext_indice ++){
        EscribeRam8(HWREGB(flash_addr+img_ext_indice));
